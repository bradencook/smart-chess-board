...
mux_select_pins = [Pin(0, Pin.OUT), Pin(1, Pin.OUT), Pin(2, Pin.OUT), Pin(3, Pin.OUT)]  # S0, S1, S2, S3
...
def read_all_signals():
    ...
    for mux_index in range(4):
        for row in range(2):  # 0 for even, 1 for odd
            all_signals.extend(read_mux(mux_index, row))
    return all_signals
